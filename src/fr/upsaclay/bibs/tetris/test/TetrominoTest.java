...
import static fr.upsaclay.bibs.tetris.model.tetromino.TetrominoShape.Rotate;
...
class TetrominoTest {
	...
	public static boolean test2Cells(TetrisCell[][] cellsExpected, TetrisCell[][] cellsActual) {
		for (int i = 0; i < cellsExpected.length; i++) {
			for (int j = 0; j < cellsExpected.length; j++) {
				if (cellsExpected[i][j] != cellsActual[i][j]) {
	...
	public void testRotate() {
		TetrisCell[][] expectedT = new TetrisCell[][]{
				{TetrisCell.EMPTY, TetrisCell.EMPTY, TetrisCell.EMPTY},
				{TetrisCell.T, TetrisCell.T, TetrisCell.T},
				{TetrisCell.EMPTY, TetrisCell.T, TetrisCell.EMPTY}
		};

		assertTrue(test2Cells(expectedT, Rotate(TRotated)));

		TetrisCell[][] expectedI = new TetrisCell[][]{
		...
		assertTrue(test2Cells(expectedI, Rotate(IRotated)));
	}
